...
def setup_logging(config):
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(f'{log_dir}/gpkg_import_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
def generate_table_name(filename, layer_name, timestamp=None):
    """生成表名"""
...
def create_metadata_tables(engine, config, layer_info, tables_created, filename, timestamp):
    """创建元数据表"""
    try:
        # 创建产品元数据表
        product_table_sql = """
...
def main():
    ...
    create_metadata_tables(engine, config, layer_info, tables_created, "testGdb", timestamp)
